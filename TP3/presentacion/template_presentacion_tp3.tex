...
% URLs de las animaciones (diapositivas 17 y 21).
...
% Simulaciones 10--13 | Resultados 14--23 | Conclusiones 24--26
...
\newcommand{\TPnavrow}{%
  \makebox[\paperwidth][l]{%
    \navblock{0.27}{Introducción}{2}{5}%
    \navblock{0.19}{Implementación}{6}{9}%
    \navblock{0.17}{Simulaciones}{10}{13}%
    \navblock{0.20}{Resultados}{14}{23}%
    \navblock{0.17}{Conclusiones}{24}{26}%
  }%
}
...
\begin{frame}{Sistema Real y Objetivo}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \maybeimage{figuras/sistema_real.jpg}{4.2cm}{Insertar imagen ilustrativa del sistema}
    \end{column}
  \end{columns}
\end{frame}

% 4
\begin{frame}{Movimiento Libre y Tiempos de Contacto}
  \small
  ...
  \textbf{Contra una pared} --- coordenada $q$, velocidad $u$, radio $R$,
  ...
  \textbf{Entre dos discos} --- con $\sigma = r_i + r_j$,
...
% 5
\begin{frame}{Resolución de los Choques}
  \small
  \textbf{Pared:} invierte normal, conserva tangencial
  ($v_x^d = -v_x^a$ en las paredes verticales, $v_y^d = -v_y^a$ en
  las horizontales).
  \vspace{0.12cm}

  \textbf{Entre dos partículas} --- por conservación del impulso:
  \begin{align*}
  ...
  {\scriptsize ($\Delta\bm{r}$, $\Delta\bm{v}$, $\sigma$: diapositiva anterior). En este
  trabajo $m_i = m_j$ para todas las partículas.
...
\begin{frame}{Parámetros}
  ...
  \begin{itemize}
    \item $N$ ($N = 100$ fijo a partir del estudio de configuraciones)
  ...
  \begin{databox}{Nro.\ de realizaciones}
    \centering
    Tiempo de ejecución: 10 \hfill $\langle t_{90}\rangle$: 5 \hfill DCM: 1
  \end{databox}
\end{frame}
...
\begin{frame}{Observables}
  ...
  \]
  {\scriptsize Si no llega a 0.9: se reporta $N_g(t_{\max})$.\par}
  \vspace{0.15cm}
...
\begin{frame}[c]{Tiempo de Ejecución en Función de $N$}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/tiempo-ejecucion-vs-n.pdf}{4.9cm}{Insertar tiempo de ejecución vs.\ $N$}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item Mesa vacía \item $t_f = 30$ s \item Realizaciones: 10}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Mesa Vacía --- Fracción de Partículas Usadas}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/fu-temporal-vacia.pdf}{4.9cm}{Insertar $F_u(t)$ y la lectura de $t_{90}$}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item $N = 100$ \item $K = 0$ \item 1--2 corridas representativas}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Un Obstáculo sobre el Eje Longitudinal}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/t90-vs-xk.pdf}{4.7cm}{Insertar $\langle t_{90}\rangle$ vs.\ $x_k$}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item $N = 100$ \item $K = 1$ \item Realizaciones: 5}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{$K$ Obstáculos de Área Total Fija}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/t90-vs-k.pdf}{4.7cm}{Insertar $\langle t_{90}\rangle$ vs.\ $K$}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item $N = 100$ \item $\sum \pi R_k^2$ constante \item Realizaciones: 5}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Configuración Elegida --- Evolución del Sistema}
  \animacionsimple{figuras/elegida_frame.png}{5.1cm}{\animacionElegidaURL}
    ...
    {\item $N = 100$ \item $K = $ COMPLETAR \item $t_{\max} = 100$ s}
\end{frame}
...
\begin{frame}[c]{Configuración Elegida --- Comparación contra la Mesa Vacía}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/fu-temporal-elegida.pdf}{4.7cm}{Insertar $F_u(t)$ de la configuración elegida y de la mesa vacía}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item $N = 100$ \item Realizaciones: 5}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Desplazamiento Cuadrático Medio y Ajuste Lineal}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/dcm-ajuste.pdf}{4.7cm}{Insertar $z(t)$ con el ajuste superpuesto}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item $N = 100$ \item Una realización \item $z = c\,t$, $D = c/2$}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Coeficiente de Difusión}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.70\textwidth}
      \centering
      \maybeimage{figuras/d-vs-t90.pdf}{4.7cm}{Insertar $D$ vs.\ $\langle t_{90}\rangle$ por configuración}
    \end{column}
    \begin{column}{0.27\textwidth}
      \parambox{\item $N = 100$ \item Barras de error solo en $\langle t_{90}\rangle$}
    \end{column}
  \end{columns}
\end{frame}
